...
def _require_demo_key(x_demo_key: str | None):
    ...
    if expected and x_demo_key != expected:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid demo key"
        )
...
def run_demo(
    ...
    # 1) Users
    coach = user_service.create_user_service(
        db, name="Coach Carla", email=f"coach_{ts}@example.com", password="Password123"
    )
    athlete = user_service.create_user_service(
        db,
        name="Athlete Alex",
        email=f"athlete_{ts}@example.com",
        password="Password123",
    )

    # 2) Club
    club = club_service.create_club_service(
        db, payload=ClubCreate(name=f"Demo Club {ts}"), user=coach
    )

    # 3) Memberships
    membership = membership_service.create_membership_service(
        db, club_id=club.id, email=athlete.email, role="coach"
    )

    # 4) Plan
    plan = plan_service.create_plan_service(
        db,
        club_id=club.id,
        me=coach,
        data=PlanCreate(
            name="Intro Plan",
            description="A starter plan created by the demo endpoint.",
            plan_type="club",
        ),
    )
    ...
    sess = session_service.create_session(
        db, club_id=club.id, plan_id=plan.id, me=coach, data=sess_in
    )
    ...
    return {
        ...
        "session": {
            ...
            "location": getattr(sess, "location", None),
        },
